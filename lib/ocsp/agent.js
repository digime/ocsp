...
Agent.prototype.createConnection = function createConnection(port,
  ...
  socket.on('secure', function() {
    return self.handleOCSPResponse(socket, stapling, options, function(err) {
      if (err)
...
Agent.prototype.handleOCSPResponse = function handleOCSPResponse(socket,
                                                                 ...
                                                                 cb) {
  if (socket.ssl === null) {
    return cb("Socket SSL cannot be NULL");
  }
  var cert = socket.ssl.getPeerCertificate(true);
  var issuer = cert.issuerCertificate;

  if (issuer && issuer.infoAccess && options.issuer && !issuer.infoAccess.includes(options.issuer)) {
    return cb("Wrong issuer");
  }
